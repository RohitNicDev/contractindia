...
/* ==========================================================================
   4. MODAL DEFAULTS — NOW WITH IMAGE FIELDS
   ========================================================================== */
const EMPTY_FORM = {
  serviceName: "",
  serviceCode: "",
...
/* ==========================================================================
   6. IMAGE INPUT COMPONENT
   ========================================================================== */
function ImageInputField({ label, value, onChange, placeholder }) {
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex-1 h-8 rounded-lg border border-slate-300 bg-slate-50 hover:bg-slate-100 text-slate-600 text-[11px] font-semibold transition-all flex items-center justify-center gap-2"
        >
        ...
        {value && (
          <button
            type="button"
            onClick={handleClear}
            className="h-8 px-3 rounded-lg border border-red-300 bg-red-50 hover:bg-red-100 text-red-600 text-[11px] font-semibold transition-all"
          >
            Clear
          </button>
        )}
      ...
      {value && (
        <div className="flex items-center gap-2 p-2 rounded-lg bg-slate-50 border border-slate-200">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-slate-500 truncate">Image selected</p>
            <p className="text-[9px] text-slate-400">{value.length} bytes</p>
          </div>
        </div>
      )}
      {!value && placeholder && (
...
/* ==========================================================================
   7. SERVICE FORM MODAL — UPDATED WITH IMAGE FIELDS
   ========================================================================== */
function ServiceFormModal({
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && initialData?._raw) {
      ...
      // Handle both PascalCase (API) and camelCase (form) properties
      setForm({
        serviceName: r.ServiceName ?? r.serviceName ?? "",
        serviceCode: r.ServiceCode ?? r.serviceCode ?? "",
        serviceDescription: r.ServiceDescription ?? r.serviceDescription ?? "",
        serviceURL: r.ServiceURL ?? r.serviceURL ?? "",
        bannerImage: r.BannerImage ?? r.bannerImage ?? "",
        thumbnailImage: r.ThumbnailImage ?? r.thumbnailImage ?? "",
        ...
        minSubscriptionLevel: r.MinSubscriptionLevel ?? r.minSubscriptionLevel ?? 0,
      });
    } else {
      setForm({ ...EMPTY_FORM });
    }
              ...
              <div className="p-5 space-y-5">
                ...
                >
                  <div className="space-y-3">
                    <ModalField label="Banner Image" span="full">
                      <ImageInputField
                        value={form.bannerImage}
                        onChange={(val) => set("bannerImage", val)}
                        placeholder="Large banner for service page"
                      />
                    </ModalField>
                    <ModalField label="Thumbnail Image" span="full">
                      <ImageInputField
                        value={form.thumbnailImage}
                        onChange={(val) => set("thumbnailImage", val)}
                        placeholder="Small thumbnail for listings"
                      />
                    </ModalField>
                  </div>
                </ModalSection>
...
/* ==========================================================================
   10. LOCAL DATA CONVERTER  (unchanged)
   ========================================================================== */
