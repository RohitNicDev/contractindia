...
import { Building2, ChevronRight, MapPin, PhoneCall, Star, LogIn } from "lucide-react";
...
function Node({ node, depth, activeId, onSelect, expandedIds, toggleExpand }) {
  ...
  return (
    <div>
      <button
        onClick={() => { onSelect(node.ServiceID); if (hasKids) toggleExpand(node.ServiceID); }}
        style={{
          paddingLeft: 12 + indent, fontSize: fs,
      ...
      >
        <span className="shrink-0 rounded-full" style={{ width: dotSize, height: dotSize, minWidth: dotSize, background: isActive ? "#fff" : c.dot, opacity: isActive ? 1 : 0.85 }} />
        <span className="flex-1 line-clamp-2 leading-snug">{node.ServiceName}</span>
        {hasKids && (
          <span className="flex items-center gap-1 shrink-0 ml-1">
            <span className="text-[9px] font-black px-1.5 py-0.5 rounded-md leading-none" style={{ background: isActive ? "rgba(255,255,255,0.2)" : "#f1f5f9", color: isActive ? "#fff" : "#64748b" }}>{node.children.length}</span>
            <ChevronRight size={11} style={{ transform: isOpen ? "rotate(90deg)" : "none", transition: "transform 0.2s", color: isActive ? "rgba(255,255,255,0.8)" : "#94a3b8" }} />
          </span>
        )}
      </button>
      <AnimatePresence initial={false}>
        {hasKids && isOpen && (
          <motion.div key="kids" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} className="overflow-hidden">
            {node.children.map((child) => (
              <Node key={child.ServiceID} node={child} depth={depth + 1} activeId={activeId} onSelect={onSelect} expandedIds={expandedIds} toggleExpand={toggleExpand} />
            ))}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {path.map((node, i) => {
        ...
        return (
          <span key={node.ServiceID} className="flex items-center gap-1">
            ...
            <button onClick={() => !isLast && onSelect(node.ServiceID)} className="text-[11px] font-bold px-2 py-0.5 rounded-lg transition-all" style={isLast ? { background: c.softBg, color: c.softText, cursor: "default" } : { color: "#94a3b8" }}>
              {node.ServiceName}
            </button>
          </span>
        );
...
function ContractorDetail({ item }) {
  ...
  const location = [item?.CityName, item?.StateName].filter(Boolean).join(", ") || "India";
  ...
  return (
    <div className="space-y-4">
      ...
      {mobile !== "—" && (
        <a href={`tel:${mobile}`} className="flex w-full items-center justify-center gap-2 py-3 rounded-2xl font-bold text-sm text-white" style={{ background: "linear-gradient(135deg,#059669,#047857)" }}>
...
function LoginPromptBody({ onLogin }) {
  return (
    <div className="py-4 text-center space-y-4">
      ...
      <div>
        <h3 className="font-black text-slate-900 text-lg">Sign in to view details</h3>
        <p className="text-sm text-slate-500 mt-1">Create a  account or log in to see contractor contact details, and more.</p>
      </div>
      <button onClick={onLogin} className="w-full py-3 rounded-2xl font-bold text-sm text-white shadow-lg" style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6)" }}>
        Sign in / Register
      </button>
    </div>
  );
...
function ContractorCard({ item, idx, onViewDetails }) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94 }} transition={{ duration: 0.22, delay: Math.min(idx * 0.03, 0.24) }} className="group bg-white rounded-3xl border border-slate-200/60 hover:border-blue-300 shadow-sm hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 overflow-hidden">
      <div className="relative h-36 m-2 rounded-2xl overflow-hidden">
        <img src={img} alt={company} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onError={(e) => { e.currentTarget.src = IMGS[idx % IMGS.length]; }} />
        ...
        {status && (
          <span className={`absolute top-2 right-2 text-[9px] font-black px-2 py-0.5 rounded-lg ${status === "Approved" ? "bg-emerald-500/90 text-white" : "bg-amber-400/90 text-white"}`}>{status}</span>
        )}
      </div>
      <div className="px-4 pb-4">
        ...
        <div className="flex items-center gap-1 text-slate-400 mb-3"><MapPin size={10} /><span className="text-[10px] truncate">{location}</span></div>
        ...
        <div className="flex gap-2">
          <button onClick={() => onViewDetails(item)} className="flex-1 bg-slate-900 hover:bg-blue-600 text-white py-2 rounded-xl text-[11px] font-bold transition-all active:scale-95">View Details</button>
          {phone && (
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden animate-pulse">
      ...
      <div className="px-4 pb-4 space-y-3 mt-2">
        ...
        <div className="flex gap-2"><div className="flex-1 h-8 bg-slate-100 rounded-xl" /><div className="flex-1 h-8 bg-slate-100 rounded-xl" /></div>
      </div>
    </div>
  );
}
...
const CompanySubServices = () => {
  ...
  useEffect(() => {
    if (!tree.length) { setActiveId(null); setExpandedIds(new Set()); return; }
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] font-sans">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 items-start">
          ...
          <aside className="sticky top-6">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              <div className="py-2 px-2 overflow-y-auto" style={{ maxHeight: "calc(100vh - 160px)", scrollbarWidth: "thin", scrollbarColor: "#e2e8f0 transparent" }}>
                {treeLoading ? (
                  <div className="space-y-2 p-2 animate-pulse">{[...Array(6)].map((_, i) => <div key={i} className="h-8 bg-slate-100 rounded-xl" />)}</div>
                ) : tree.length === 0 ? (
                  <p className="text-center text-xs text-slate-400 py-8">No categories found</p>
                ) : (
                  tree.map((node) => (
                    <Node key={node.ServiceID} node={node} depth={0} activeId={activeId} onSelect={handleSelect} expandedIds={expandedIds} toggleExpand={toggleExpand} />
                  ))
                )}
              </div>
            </div>
          ...
          <section className="min-w-0">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-900 flex items-center gap-2 flex-wrap">
                  ...
                  {!contractorsLoading && activeId && (
                    <span className="text-sm font-bold bg-slate-200 text-slate-600 px-2 py-0.5 rounded-lg">{contractors.length}</span>
                  )}
                </h2>
                {activeNode && (
                  <p className="text-[10px] text-slate-400 mt-0.5 font-medium">
                    ID #{activeNode.ServiceID}{activeNode.children?.length > 0 ? ` · ${activeNode.children.length} sub-categories` : " · leaf"}
                  </p>
                )}
              </div>
            </div>
              ...
              {contractorsLoading ? (
                ...
                <AnimatePresence mode="popLayout">
                  {contractors?.map((item, idx) => (
                    <ContractorCard key={item?.userId ?? idx} item={item} idx={idx} onViewDetails={isLoggedIn?handleViewDetails:handleConfirmView} />
                  ))}
                </AnimatePresence>
              )}
            </motion.div>
            ...
            {!contractorsLoading && contractors.length === 0 && activeId && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white rounded-[40px] border border-dashed border-slate-200 py-20 text-center mt-4">
      ...
      <CommonModal
        ...
        hideFooter
      >
        <LoginPromptBody onLogin={() => { setShowLoginModal(false); navigate("/login"); }} />
      </CommonModal>
    </div>
  );
};
