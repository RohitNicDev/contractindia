...
/* ── Category ── */
const catGetApi = async () => {
  const r = await DocumentCategoryGet();
  return r?.data ?? r ?? [];
};
const catSaveApi = async (p) => {
  const r = await DocumentCategorySave(p);
  return r ?? {};
};
const catUpdateApi = async (p) => {
  const r = await DocumentCategoryUpdate(p);
  return r ?? {};
};
const catDeleteApi = async (id) => {
  const r = await DocumentCategoryDelete(id);
  return r ?? {};
};

/* ── SubCategory ── */
const subGetApi = async () => {
  const r = await DocumentSubCategoryGet();
  return r?.data ?? r ?? [];
};
const subSaveApi = async (p) => {
  const r = await DocumentSubCategorySave(p);
  return r ?? {};
};
const subUpdateApi = async (p) => {
  const r = await DocumentSubCategoryUpdate(p);
  return r ?? {};
};
const subDeleteApi = async (id) => {
  const r = await DocumentSubCategoryDelete(id);
  return r ?? {};
};
...
const DocumentCategoryManager = () => {
  ...
  const [catViewModal, setCatViewModal] = useState({
    open: false,
    record: null,
  });
  ...
  const openCatCreate = () => {
    setCatSelectedId(0);
    catForm.resetFields();
    catForm.setFieldsValue({ isActive: true });
  ...
  const { mutate: catDelete, isPending: catDeleting } = useMutation({
    ...
    onSuccess: (res) => {
      ...
      setCatDeleteModal({ open: false, record: null });
      catRefetch();
    },
  ...
  const [subDeleteModal, setSubDeleteModal] = useState({
    open: false,
    record: null,
  });
  const [subViewModal, setSubViewModal] = useState({
    open: false,
    record: null,
  });
  ...
  const openSubCreate = () => {
    setSubSelectedId(0);
    subForm.resetFields();
    subForm.setFieldsValue({ isActive: true });
    setSubFormModal(true);
  };
  const openSubEdit = (r) => {
    const id =
      r?.DocumentSubCategoryID ?? r?.documentSubCategoryID ?? r?.id ?? 0;
    setSubSelectedId(id);
    subForm.setFieldsValue({
      documentSubCategoryName:
        r?.DocumentSubCategoryName ?? r?.documentSubCategoryName ?? "",
      documentCategoryID:
        r?.DocumentCategoryID ?? r?.documentCategoryID ?? undefined,
      isActive: resolveActive(r),
    });
    setSubFormModal(true);
  };
  ...
  const { mutate: subDelete, isPending: subDeleting } = useMutation({
    ...
    onSuccess: (res) => {
      ...
      setSubDeleteModal({ open: false, record: null });
      subRefetch();
    },
    onError: (e) => toast.error(e?.message || "Unable to delete."),
  });
    ...
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_rgba(139,92,246,0.07),_transparent_55%),#f8fafc] p-4 sm:p-6">
      <div className="mx-auto  space-y-5">
        ...
        <CustomHeading
          ...
          actions={
            <div className="flex items-center gap-2">
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 ${(isCat ? catFetching : subFetching) ? "animate-spin" : ""}`}
                />
              </button>
              <button
          ...
          {catViewModal.record && (
            <div className="mt-3 space-y-4">
              <DetailRow
                label="Category Name"
                value={
                  catViewModal.record?.DocumentCategoryName ??
                  catViewModal.record?.documentCategoryName
                }
              />
              <DetailRow
              ...
              <DetailRow
                ...
                custom={
                  <Badge
                    color={
                      resolveActive(catViewModal.record) ? "green" : "yellow"
                    }
                  >
              ...
              <DetailRow
                label="Created Date"
                value={formatDate(
                  catViewModal.record?.CreatedDate ??
                    catViewModal.record?.createdDate,
                )}
              />
              <DetailRow
                label="Last Updated"
                value={formatDate(
                  catViewModal.record?.UpdatedDate ??
                    catViewModal.record?.updatedDate,
                )}
              />
            </div>
        ...
        <CommonModal
          ...
          onConfirm={() => {
            const id =
              catDeleteModal.record?.DocumentCategoryID ??
              catDeleteModal.record?.documentCategoryID ??
              catDeleteModal.record?.id;
            if (!id) {
              toast.error("Cannot delete — no valid ID.");
              return;
            }
            catDelete(id);
          }}
        >
          <p className="text-sm text-slate-600 mt-1">
            ...
            <span className="font-bold text-slate-900">
              "
              {catDeleteModal.record?.DocumentCategoryName ??
                catDeleteModal.record?.documentCategoryName}
              "
            </span>
            ?
          ...
          {subViewModal.record && (
            <div className="mt-3 space-y-4">
              <DetailRow
                ...
                value={
                  subViewModal.record?.DocumentSubCategoryName ??
                  subViewModal.record?.documentSubCategoryName
                }
              />
              <DetailRow
              ...
              <DetailRow
                ...
                value={getCatName(
                  subViewModal.record?.DocumentCategoryID ??
                    subViewModal.record?.documentCategoryID,
                )}
              />
              <DetailRow
                ...
                custom={
                  <Badge
                    color={
                      resolveActive(subViewModal.record) ? "green" : "yellow"
                    }
                  >
              ...
              <DetailRow
                label="Created Date"
                value={formatDate(
                  subViewModal.record?.CreatedDate ??
                    subViewModal.record?.createdDate,
                )}
              />
              <DetailRow
                label="Last Updated"
                value={formatDate(
                  subViewModal.record?.UpdatedDate ??
                    subViewModal.record?.updatedDate,
                )}
              />
            </div>
        ...
        <CommonModal
          ...
          onConfirm={() => {
            const id =
              subDeleteModal.record?.DocumentSubCategoryID ??
              subDeleteModal.record?.documentSubCategoryID ??
              subDeleteModal.record?.id;
            if (!id) {
              toast.error("Cannot delete — no valid ID.");
              return;
            }
        ...
        >
          <p className="text-sm text-slate-600 mt-1">
            ...
            <span className="font-bold text-slate-900">
              "
              {subDeleteModal.record?.DocumentSubCategoryName ??
                subDeleteModal.record?.documentSubCategoryName}
              "
            </span>
            ?
