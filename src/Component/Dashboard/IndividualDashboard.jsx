...
const NAV = [
  ...
  // { id: "credits", label: "My Credits", icon: Wallet },
];
    ...
    <div className={`${glassCard} p-6`}>
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            Review your active and past subscription plans.
          </p>
        </div>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        {subscriptions.map((sub) => (
          ...
          >
            ...
            <p className="text-sm text-slate-700 font-black mb-2">
              {sub.price}
            </p>
  ...
  const services = [
    {
      ...
      price: "₹2,500",
      icon: "🏗️",
      desc: "Expert PMC & planning consultation",
    },
    {
      name: "Legal Contracts",
      price: "₹1,800",
      icon: "⚖️",
      desc: "Contract drafting & legal review",
    },
    {
      name: "Tender Services",
      price: "₹3,200",
      icon: "📋",
      desc: "Tender filing & bid management",
    },
    {
      name: "Assets Management",
      price: "₹4,000",
      icon: "🏢",
      desc: "Infrastructure asset tracking",
    },
    {
      name: "Brand Development",
      price: "₹5,500",
      icon: "🎨",
      desc: "Brand identity & marketing",
    },
    {
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {services.map((s, i) => (
          <motion.div
            key={i}
            whileHover={{ y: -4 }}
            className="p-4 rounded-2xl border border-slate-200/80 bg-white/80 hover:border-indigo-200 hover:shadow-lg transition-all cursor-pointer group"
          >
    ...
    <div className={`${glassCard} p-6`}>
      ...
      <div className="rounded-2xl bg-gradient-to-br from-indigo-50 to-violet-50 border border-indigo-100 p-5 mb-4">
        ...
        <h4 className="font-black text-slate-900 text-base mb-1">
          May 2026 — Construction Market Insights
        </h4>
      ...
      <p className="text-xs text-slate-400">
        Upgrade to paid subscription to access all newsletters.
      </p>
    </div>
  );
}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">
            Update account details, security, and subscriptions.
          </p>
        </div>
      </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50/90 p-5">
          ...
          <label className="block mb-4">
            ...
            <textarea
              ...
              rows={3}
              className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 resize-none"
            />
          </label>
          <motion.button
            whileHover={{ scale: 1.02 }}
...
export default function IndividualDashboard() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "history":
        return <History />;
      case "settings":
        return <SettingsPanel user={user} onUpdate={setUser} />;
      default:
        return <Overview user={user} />;
    }
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-indigo-50/30 to-violet-50/20">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-60 flex flex-col bg-white/80 backdrop-blur-2xl border-r border-indigo-100/60 shadow-[4px_0_24px_rgba(99,102,241,0.06)] lg:relative lg:translate-x-0 transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        <div className="flex h-16 items-center gap-3 border-b border-indigo-100/60 px-4">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-violet-500 text-sm font-black text-white shadow-md">
            {initials}
          </div>
          <div>
      ...
      <div className="relative z-10 flex flex-1 flex-col overflow-hidden">
        <header className="flex h-16 items-center gap-4 bg-white/80 backdrop-blur-xl border-b border-indigo-100/60 px-4 sm:px-6">
          <button
            className="lg:hidden rounded-xl border border-slate-200 bg-white p-2 text-slate-500"
