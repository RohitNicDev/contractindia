...
const serviceMasterSaveApi = async (payload) =>
  (await ServiceMasterSave({
    serviceID: 0, parentServiceID: payload?.parentServiceID ?? 0,
...
const countTotalNodes = (nodes) => {
  let t = 0;
  nodes.forEach((n) => { t++; if (n.children?.length) t += countTotalNodes(n.children); });
  return t;
};
const gatherAllNodeIds = (nodes, out = []) => {
  nodes.forEach((n) => { out.push(n.id); if (n.children?.length) gatherAllNodeIds(n.children, out); });
  return out;
};
const filterTreeHierarchy = (nodes, term) =>
  nodes.map((node) => {
    ...
    if (self || sub.length) return { ...node, children: sub, isSearchHighlight: self };
    return null;
  }).filter(Boolean);
...
/* ==========================================================================
   ...
   ========================================================================== */
const EMPTY_FORM = {
  serviceName: "", serviceCode: "", serviceDescription: "", serviceURL: "",
  metaTitle: "", metaKeywords: "", metaDescription: "", searchKeywords: "",
  displayOrder: 0, isFeatured: 0, displayOnHomePage: 0, isActive: 1,
  isPopular: 0, isVerifiedRequired: 0, minSubscriptionLevel: 0,
};
...
function ServiceFormModal({ isOpen, onClose, onSubmit, mode, parentName, initialData, isLoading }) {
  ...
  useEffect(() => {
    ...
    if (mode === "edit" && initialData?._raw) {
      ...
      setForm({
        serviceName: r.serviceName ?? "", serviceCode: r.serviceCode ?? "",
        serviceDescription: r.serviceDescription ?? "", serviceURL: r.serviceURL ?? "",
        metaTitle: r.metaTitle ?? "", metaKeywords: r.metaKeywords ?? "",
        metaDescription: r.metaDescription ?? "", searchKeywords: r.searchKeywords ?? "",
        displayOrder: r.displayOrder ?? 0, isFeatured: r.isFeatured ?? 0,
        displayOnHomePage: r.displayOnHomePage ?? 0, isActive: r.isActive ?? 1,
        isPopular: r.isPopular ?? 0, isVerifiedRequired: r.isVerifiedRequired ?? 0,
  ...
  const modalTitle = mode === "addRoot" ? "Add Root Service"
    : mode === "addChild" ? `Add Child — "${parentName}"` : `Edit — "${parentName}"`;
  const accent = mode === "addRoot" ? "from-violet-600 to-fuchsia-600"
    : mode === "addChild" ? "from-sky-500 to-blue-600" : "from-amber-500 to-orange-500";
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div key="md" initial={{ opacity: 0, scale: 0.94, y: 24 }}
            animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 16 }}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {mode === "addChild" && <p className="text-[11px] text-white/70 mt-0.5">Nested under selected parent</p>}
                    </div>
                  </div>
                  <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-xl bg-white/20 text-white hover:bg-white/30 transition-all">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              </div>
              <div className="p-5 space-y-5">
                <ModalSection label="Basic Information" icon={<Tag className="h-3.5 w-3.5" />}>
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <ModalField label="Service Name *" span="full">
                      <input ref={firstInputRef} value={form.serviceName}
                        onChange={(e) => set("serviceName", e.target.value)}
                        onKeyDown={(e) => e.key === "Enter" && handleSubmit()}
                        placeholder="e.g. EPC Consultancy" className="modal-input" />
                    </ModalField>
                    <ModalField label="Service Code">
                      <input value={form.serviceCode} onChange={(e) => set("serviceCode", e.target.value)} placeholder="e.g. EPC-001" className="modal-input" />
                    </ModalField>
                    <ModalField label="Display Order">
                      <input type="number" value={form.displayOrder} onChange={(e) => set("displayOrder", Number(e.target.value))} className="modal-input" />
                    </ModalField>
                  </div>
                  <ModalField label="Description">
                    <textarea value={form.serviceDescription} onChange={(e) => set("serviceDescription", e.target.value)}
                      rows={2} placeholder="Short description..." className="modal-input resize-none" />
                  </ModalField>
                </ModalSection>
                ...
                <ModalSection label="Links & URLs" icon={<Link className="h-3.5 w-3.5" />}>
                  <ModalField label="Service URL">
                    <input value={form.serviceURL} onChange={(e) => set("serviceURL", e.target.value)} placeholder="https://..." className="modal-input" />
                  </ModalField>
                </ModalSection>
                <ModalSection label="SEO Metadata" icon={<Hash className="h-3.5 w-3.5" />}>
                  <div className="space-y-3">
                    <ModalField label="Meta Title"><input value={form.metaTitle} onChange={(e) => set("metaTitle", e.target.value)} placeholder="Page title for SEO" className="modal-input" /></ModalField>
                    <ModalField label="Meta Keywords"><input value={form.metaKeywords} onChange={(e) => set("metaKeywords", e.target.value)} placeholder="keyword1, keyword2..." className="modal-input" /></ModalField>
                    <ModalField label="Meta Description"><textarea value={form.metaDescription} onChange={(e) => set("metaDescription", e.target.value)} rows={2} placeholder="Brief SEO description" className="modal-input resize-none" /></ModalField>
                    <ModalField label="Search Keywords"><input value={form.searchKeywords} onChange={(e) => set("searchKeywords", e.target.value)} placeholder="Internal search tags..." className="modal-input" /></ModalField>
                  </div>
                </ModalSection>
              </div>
              <div className="border-t border-slate-100 px-5 py-4 flex items-center justify-end gap-2">
                <button type="button" onClick={onClose} className="h-9 rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold text-slate-600 hover:bg-slate-50 transition-all">Cancel</button>
                <button type="button" onClick={handleSubmit} disabled={isLoading}
                  ...
                  {isLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : mode === "edit" ? <Check className="h-3.5 w-3.5" /> : <Plus className="h-3.5 w-3.5" />}
      ...
      {isOpen && (
        <>
          <motion.div key="dbd" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 bg-black/30 backdrop-blur-sm" onClick={onClose} />
          <motion.div key="dmd" initial={{ opacity: 0, scale: 0.94, y: 16 }}
            animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 16 }}
            ...
            <div className="w-full max-w-sm rounded-3xl bg-white shadow-2xl border border-slate-200/70 overflow-hidden">
              <div className="bg-gradient-to-r from-red-500 to-rose-600 p-5 flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-white/20 text-white"><Trash2 className="h-4 w-4" /></div>
                <div><h2 className="text-sm font-extrabold text-white">Delete Service</h2><p className="text-[11px] text-white/70 mt-0.5">This action cannot be undone</p></div>
                <button onClick={onClose} className="ml-auto flex h-8 w-8 items-center justify-center rounded-xl bg-white/20 text-white hover:bg-white/30 transition-all"><X className="h-4 w-4" /></button>
              </div>
              <div className="p-5">
                <p className="text-sm text-slate-600">Are you sure you want to delete <span className="font-bold text-slate-900">"{nodeName}"</span>? All child services will also be removed.</p>
              </div>
              <div className="border-t border-slate-100 px-5 py-4 flex items-center justify-end gap-2">
                <button type="button" onClick={onClose} className="h-9 rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold text-slate-600 hover:bg-slate-50 transition-all">Cancel</button>
                <button type="button" onClick={onConfirm} disabled={isLoading}
                  ...
                  {isLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}Delete
                </button>
              </div>
            </div>
          </motion.div>
...
const ServiceListing = ({
  ...
  const handleSearchChange = (e) => {
    ...
    if (v.trim()) {
      ...
      if (ids.length) setExpandedNodeIds((p) => Array.from(new Set([...p, ...ids])));
    }
  };
  ...
  return (
    <>
      <ServiceFormModal isOpen={modal.open} onClose={closeModal} onSubmit={handleModalSubmit}
        mode={modal.mode} parentName={modal.targetNode?.name ?? ""}
        initialData={modal.targetNode} isLoading={isSaving || isUpdating} />
      <DeleteConfirmModal isOpen={deleteModal.open} onClose={closeDelete}
        onConfirm={handleConfirmDelete} nodeName={deleteModal.targetNode?.name ?? ""}
