  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        ...
        <span className="text-xs text-slate-400 ml-1">/{yearly ? "yr" : "mo"}</span>
        {yearly && (
...
/* ── SCREEN 1: Plans ────────────────────────────────────────────────────── */
function PlansPage({ onNext, currentPlanId }) {
      ...
      <div className="flex-shrink-0 border-t border-slate-100 px-6 py-4 flex items-center justify-between bg-white">
        <div>
          ...
          <p className="text-[11px] text-slate-400">
            ₹{price(selected, yearly)}/{yearly ? "yr" : "mo"} · billed {yearly ? "annually" : "monthly"}
          </p>
        </div>
        <button
          onClick={() => onNext({ plan: selected, yearly })}
          className="flex items-center gap-2 h-9 px-5 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white text-xs font-black shadow-md shadow-violet-200 hover:scale-[1.02] transition-transform"
        >
        ...
        <div className={`rounded-2xl border-2 ${plan.border} ${plan.lightBg} p-4`}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">Billed {yearly ? "annually" : "monthly"}</p>
            </div>
            <div className="ml-auto text-right">
              <p className={`font-black text-base ${plan.text}`}>₹{total}</p>
              <p className="text-[10px] text-slate-400">/{yearly ? "yr" : "mo"}</p>
            </div>
          </div>
...
function SuccessPage({ selection, onClose, onDashboard }) {
  ...
  yearly ? expires.setFullYear(today.getFullYear() + 1) : expires.setMonth(today.getMonth() + 1);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-8 py-10 text-center">
      ...
      >
        <plan.icon className={`h-4 w-4 ${plan.text}`} />
        <span className={`text-xs font-black ${plan.text}`}>
          {plan.name} · ₹{price(plan, yearly)}/{yearly ? "yr" : "mo"}
        </span>
...
export default function SubscriptionPlansFlow({
  ...
  dashboardPath = "/dashboard",   // override to match your route
}) {
  ...
  const handleNext = (sel) => { setSelection(sel); setStep(1); };

  const handleConfirm = () => {
    setIsLoading(true);
    /* replace with your real payment API call */
    setTimeout(() => { setIsLoading(false); setStep(2); }, 2200);
  };
              ...
              <AnimatePresence mode="wait">
                {step === 0 && (
                  <motion.div key="s0" className="flex-1 flex flex-col overflow-hidden"
                    ...
                    <PlansPage onNext={handleNext} currentPlanId={currentPlanId} />
                  </motion.div>
                )}
                {step === 1 && selection && (
